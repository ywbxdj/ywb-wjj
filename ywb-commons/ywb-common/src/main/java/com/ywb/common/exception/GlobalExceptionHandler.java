...
import com.ywb.common.domain.ErrorMessage;
import com.ywb.common.util.I18nUtils;
import com.ywb.common.util.StringUtils;
...
public class GlobalExceptionHandler {
    ...
    @PostConstruct
    public void init() {
        logger.info("init global exception handler!");
    }
    ...
    public ErrorMessage handleAllException(Exception ex) {
        ...
        return new ErrorMessage(String.valueOf(HttpStatus.BAD_REQUEST.value()), ex.getMessage());
    }
    ...
    public ErrorMessage handleHttpMessageNotReadableException(HttpMessageNotReadableException ex) throws IOException {
        logger.error(ex.getMessage(),ex);
        String errorCode =  this.appNo + "B" +"-100";
        return new ErrorMessage(errorCode, "HTTP参数转换异常", ex.getMessage());
    }
